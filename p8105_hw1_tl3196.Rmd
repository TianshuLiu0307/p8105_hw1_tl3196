...
After using the function as.numeric, the values of logical variables ``r pull(data_type_df, logical)`` turns out to be ``r as.numeric(pull(data_type_df, logical))``, which can be used to calculate mean; 

The character variables``r pull(data_type_df, character)`` turns out to be ``r as.numeric(pull(data_type_df, character))``, which are all NAs so the mean cannot be calculated;

The factor variables``r pull(data_type_df, factor)`` turns out to be ``r as.numeric(pull(data_type_df, factor))``, which can also be calculated for the mean.
